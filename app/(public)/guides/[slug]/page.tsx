...
export async function generateMetadata({
  ...
  if (!detail) return {};
  return articleMetadata(detail, `/guides/${slug}`);
}
...
export default async function GuidePage({
  ...
  const detail = await getArticleBySlug(slug);
  if (!detail) notFound();
  // Non-guide articles live under /articles — keep one canonical URL.
  const canonical = articleHref({
    slug,
    articleType: detail.article.article_type,
  });
  if (canonical !== `/guides/${slug}`) permanentRedirect(canonical);
  ...
  return (
    <>
      ...
      <ArticleView
        detail={detail}
        breadcrumbRoot={{ label: "Guides", href: "/guides" }}
      />
    </>
  );
}
